    ...
    val f = setupFuture.flatMap { _ =>
      ...
      val insertAndPrintAction: DBIO[Unit] = insertAction.map {
        coffeesInsertResult =>
          ...
          coffeesInsertResult foreach {
            numRows => println(s"Inserted $numRows rows into the Coffees table")
          }
      }
      //#insertAction
      ...
      val combinedAction: DBIO[Seq[(Int, String, String, String, String, String)]] = insertAndPrintAction.andThen(allSuppliersAction)
      val combinedFuture: Future[Seq[(Int, String, String, String, String, String)]] = db.run(combinedAction)

      combinedFuture.map {
        allSuppliers => allSuppliers.foreach( println )
      }
    }.flatMap { _ =>
